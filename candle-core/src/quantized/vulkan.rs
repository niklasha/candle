...
impl QVulkanStorage {
    ...
    pub fn fwd(
        ...
        // second dimension as the implementation in candle-vulkan-kernels doesn't handle batch
        // properly.
        let m = match dst_shape.len() {
            3 => dst_shape[0] * dst_shape[1],
            2 => dst_shape[0],
            n => crate::bail!("Invalid rank {n} for quantized matmul vulkan"),
        };
        ...
        for batch_id in 0..m {
            candle_vulkan_kernels::call_quantized_matmul_mv_t(
                device.device(),
                &command_buffer,
                device.kernels(),
                self.dtype.into(),
        ...
        let dst_storage =
            crate::VulkanStorage::new(dst, device, dst_shape.elem_count(), DType::F32);
